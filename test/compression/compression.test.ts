...
describe("Test MemoryStore", () => {
    const hStore = new HTTPStore("http://localhost:3000/");
    ...
    it("Can open the simple nested fixture", async () => {
        const z = await openArray({ store: hStore, path: "simple_nested.zarr" });
        expect(z.shape).toEqual([8, 8]);
        expect(await z.get([0, 0])).toEqual(1);
        expect(await z.get([0, 1])).toEqual(2);
        expect(await z.get([7, 7])).toEqual(3);
        expect(await z.get([4, 4])).toEqual(0);
        expect(z.meta.dimension_separator).toEqual('/');
    });
