...
describe("NestedArray interface", () => {
    ...
    it("errors given invalid buffer size", () => {
        expect(() => new NestedArray(new ArrayBuffer(16), [2], "<i8")).not.toThrowError();
        expect(() => new NestedArray(new ArrayBuffer(7), [2], "<i8")).toThrowError();
    ...
    it("errors when not passing required params", () => {
        ...
        expect(() => new NestedArray(new ArrayBuffer(8) as any, undefined, "<i8")).toThrowError();
    });
